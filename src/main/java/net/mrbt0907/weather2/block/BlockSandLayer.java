...
public class BlockSandLayer extends Block
{
	...
	{
		return state.getValue(LAYERS) >=8 ? Blocks.SAND.getItemDropped(state, rand, fortune) : ItemRegistry.itemSandLayer;
	}
	...
	{
		return state.getValue(LAYERS) >= 8 ? Blocks.SAND.quantityDropped(state, fortune, random) : state.getValue(LAYERS);
	}
