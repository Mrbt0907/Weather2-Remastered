...
public class EntityLightningBolt extends EntityWeatherEffect
{
    ...
    public int fireLifeTime = ConfigStorm.lightning_bolt_fire_lifetime;
    public int fireChance = ConfigStorm.lightning_bolt_sets_fire_10_in_x;
    ...
    public EntityLightningBolt(World par1World, double par2, double par4, double par6)
    {
        ...
        if (ConfigStorm.enable_lightning_bolt_fires) {
    ...
    public void onUpdate()
    {
        ...
        if (this.lightningState < 0)
        {
            ...
            else if (this.lightningState < -this.rand.nextInt(10))
            {
                ...
                if (ConfigStorm.enable_lightning_bolt_fires && !this.world.isRemote && rand.nextInt(fireChance) == 0 && this.world.getGameRules().getBoolean("doFireTick") && this.world.isAreaLoaded(new BlockPos(MathHelper.floor(this.posX), MathHelper.floor(this.posY), MathHelper.floor(this.posZ)), 10))
                {
    ...
    public void updateSoundEffect() {
    	...
    	if (mc.player != null && mc.player.getDistance(this) < ConfigStorm.max_lightning_bolt_distance) {
    		this.world.playSound(this.posX, this.posY, this.posZ, SoundEvents.ENTITY_LIGHTNING_THUNDER, SoundCategory.WEATHER, 64.0F * (float)ConfigVolume.lightning, 0.8F + this.rand.nextFloat() * 0.2F, false);
