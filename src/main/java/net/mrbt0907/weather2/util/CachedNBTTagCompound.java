...
public class CachedNBTTagCompound {
	...
	public UUID getUUID(String key) {
		if (!newData.hasKey(key))
			newData.setUniqueId(key, cachedData.getUniqueId(key));
		return newData.getUniqueId(key);
	}

	public void setUUID(String key, UUID newVal) {
		if (!cachedData.hasKey(key) || cachedData.getUniqueId(key) != newVal || forced)
			newData.setUniqueId(key, newVal);
		cachedData.setUniqueId(key, newVal);
	}
