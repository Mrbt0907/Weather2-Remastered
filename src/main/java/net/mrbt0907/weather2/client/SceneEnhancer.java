...
import net.mrbt0907.weather2.client.entity.particle.ParticleFish;
...
public class SceneEnhancer implements Runnable {
	                    ...
	                    if (block == null || (block.getMaterial(block.getDefaultState()) != Material.WATER && block.getMaterial(block.getDefaultState()) != Material.LEAVES)) {
							...
							if (lastPlayTime < System.currentTimeMillis()) {
								if (cCor.block == SOUNDMARKER_WATER) {
									...
									mc.world.playSound(cCor.toBlockPos(), SoundRegistry.get("env.waterfall"), SoundCategory.AMBIENT, (float)ConfigVolume.waterfall, 0.75F + (rand.nextFloat() * 0.05F), false);
									...
									if (windSpeed > 0.2F) {
										...
										mc.world.playSound(cCor.toBlockPos(), SoundRegistry.get("env.wind_calmfade"), SoundCategory.AMBIENT, (float)(windSpeed * 4F * ConfigVolume.trees), 0.70F + (rand.nextFloat() * 0.1F), false);
										...
										if (mc.world.rand.nextInt(15) == 0) {
											...
											mc.world.playSound(cCor.toBlockPos(), SoundRegistry.get("env.wind_calmfade"), SoundCategory.AMBIENT, (float)(windSpeed * 2F * ConfigVolume.trees), 0.70F + (rand.nextFloat() * 0.1F), false);
										}
											//System.out.println("play leaves sound at: " + cCor.posX + " - " + cCor.posY + " - " + cCor.posZ + " - windSpeed: " + windSpeed);
										//}
									}
                ...
                for (int yy = curY - (hsize / 2); yy < curY + hsize; yy++)
                {
                    for (int zz = curZ - hsize; zz < curZ + hsize; zz++)
                    {
                        ...
                        if (block != null) {
                            ...
                            } else if (ConfigVolume.trees > 0 && ((block.getMaterial(block.getDefaultState()) == Material.LEAVES))) {
                            	boolean proxFail = false;
                				for (int j = 0; j < soundLocations.size(); j++) {
	...
	public void tickParticlePrecipitation() {
		...
		if (ConfigParticle.enable_precipitation) {
			...
			if (entP.posY >= StormObject.weather_layer_0) return;

			WeatherSystemClient weatherMan = ClientTickHandler.weatherManager;
			if (weatherMan == null) return;
			WindManager windMan = weatherMan.windMan;
            ...
            if (biomegenbase != null && (biomegenbase.canRain() || biomegenbase.getEnableSnow()))
            {
				...
				if (curPrecipVal > 0) {
					...
					int spawnCount;
					int spawnNeed = (int)(curPrecipVal * 40F * ConfigParticle.precipitation_particle_rate * particleAmp);
					...
					if (entP.world.getBiomeProvider().getTemperatureAtHeight(temperature, precipitationHeight) >= 0.15F) {
						...
						if (groundSplash == true && curPrecipVal > 0.15) {
							for (int i = 0; i < 30F * curPrecipVal * ConfigParticle.precipitation_particle_rate * particleAmp * 4F; i++) {
								BlockPos pos = new BlockPos(
										entP.posX + rand.nextInt(spawnAreaSize) - (spawnAreaSize / 2),
						...
						if (downfall == true && curPrecipVal > 0.32) {
							...
							for (int i = 0; i < 2F * curPrecipVal * ConfigParticle.precipitation_particle_rate; i++) {
								BlockPos pos = new BlockPos(
										entP.posX + rand.nextInt(spawnAreaSize) - (spawnAreaSize / 2),
										entP.posY + 5 + rand.nextInt(15),
										entP.posZ + rand.nextInt(spawnAreaSize) - (spawnAreaSize / 2));
	...
	{
		if (WeatherSystemClient.closestStormCached == null || entP.world.getTotalWorldTime() % 5 == 0) {
			//Minecraft mc = FMLClientHandler.instance().getClient();
			...
			WeatherSystemClient.closestStormCached = (StormObject) ClientTickHandler.weatherManager.getClosestStorm(plPos, maxStormDist, WeatherEnum.Type.TORNADO.getStage(), 10, WeatherEnum.Type.SANDSTORM);
		}

		return WeatherSystemClient.closestStormCached;
	}
	...
	public void profileSurroundings()
    {
        ...
        WeatherSystemClient manager = ClientTickHandler.weatherManager;
        ...
        if ((!ConfigParticle.enable_falling_leaves && !ConfigParticle.enable_waterfall_splash)/* || weatherMan.wind.strength < 0.10*/)
        {
            return;
        }
                        ...
                        //{
							...
							block.getMaterial(block.getDefaultState()) == Material.PLANTS))
                            {
                            	...
                            	if (worldRef.rand.nextInt(spawnRate) == 0)
                                {
	                                ...
	                                if (ConfigParticle.enable_falling_leaves) {
                            ...
                            } else if (ConfigParticle.enable_fire_particle && (block != null && block == Blocks.FIRE/*block.getMaterial() == Material.fire*/)) {
                            	lastTickFoundBlocks++;
